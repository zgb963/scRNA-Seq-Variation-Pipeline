...
def runCellRanger(SRRs):
    ...
    transciptome = current_path + '/mouse_genome/refdata-gex-mm10-2020-A'        #path to mouse genome
    samples = ' '.join([str(SRR) + '.1' for SRR in SRRs])
    samples = samples.replace(' ', ',')
    cellranger_cmd = 'cellranger count --id=' + id + ' --fastqs=' + fastqs_path + ' --sample=' + samples + ' --transcriptome=' + transciptome
